...
const Addresses = () => {
  ...
  return (
    <div className="min-h-screen bg-app-cream">
       <div className="amx-w-7xl mx-auto sm:px-6 lg:px-8 py-8">
          
          {/* page header */}
          <div className="flex items-center justify-between mb-8">
            <h1 className="text-2xl font-semibold text-app-green ">My Addresses</h1>
            <button onClick={()=> {resetForm(); setShowForm(true)}} className="px-4 py-2 bg-app-green text-white tetx-sm font-semibold rounded-xl hover:bg-app-green-light transition-colors flex items-center gap-2">
              <PlusIcon className="size-4" /> Add Address
            </button>
          </div>
          
          {/* form model */}
             {showForm && <AddressForm resetForm={resetForm} handleSumbit={handleSumbit} form={form} setForm={setForm} editingId={editingId} />}

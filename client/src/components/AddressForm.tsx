...
const AddressForm = ({ resetForm, handleSumbit, form, setForm, editingId }: any) => {

    return (
        <>
            {/* overlay  */}
            <div className="fixed inset-0 bg-balck/40 z-50" />

            {/* form container */}
            <div onClick={resetForm} className="fixed inset-0 z-50 flex-center p-4">
                <form onClick={e => e.stopPropagation()} onSubmit={handleSumbit} className="bg-white rounded-2xl p-6 w-full max-w-lg animate-fade-in">
                    ...
                    <div className="space-y-4">
                        <div>
                            <label className="blcok text-sm font-medium text-app-green mb-1.5
                    5">Label</label>
                    ...
                    <button type="submit" className="mt-6 w-full py-3 bg-app-green text-white font-semibold rounded-xl hover:bg-app-green-light transition-colors ">
                       {editingId ? "Updated Address" : "Save Address"}
                    </button>
                </form>
            </div>
        </>

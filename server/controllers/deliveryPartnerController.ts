...
export const loginPartner = async (req: Request, res: Response) => {
 ...
 const {password: _, ...parentData} = partner;

 res.json({partner: parentData,token})

}


// get assigned deliveries
// get /api/delivery/my-deliveres
...
// get /api/delivery/my-deliverys/:id
...
// complete deliery with otp
// put /api/delivery/my-deliveries/:id/complete
export const compleyeDelivery = async (req: Request, res: Response) => {
  const {otp} = req.body;
  const order =await prisma.order.findFirst({
    where: {id: req.params.id as string, deliveryPartnerId: req.partner!.id}
  })
...
export const updateDeliveryStatus = async (req: Request, res: Response) => {
  ...
  const alloedStatuses = ["Packed", "Out for Delivery"];

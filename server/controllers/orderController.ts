...
// look up actual prices from the databses
const productsIds = items.map((i: any)=> i.product);
const products = await prisma.product.findMany({where: {id: {in: productsIds}}})
const productMap: Record<string, (typeof products)[0]> = {}

products.forEach((p: any)=> (productMap[p.id] = p))

// check if products is instock
...
// updae order status (admin)
// put /api/orders/:id/staus
export const updateOrderStatus =  async (req: Request, res: Response)=>{
  ...
  history.push({status, note: note || `Order 
    ${status.tolowerCase()}`, timestamp: new Date()})

...
// monthly offers email (1st of very month - padyday)
 const sendMonthlyOffers = inngest.createFunction({
  ...
  triggers: [cron("0 10 1 * *")]
    ...
    // send in bacthes of to aviod overwhelming mail server
    const batchSize = 10;
...
const autoAssignRider = inngest.createFunction({
    ...
    triggers: [{event: "order/palced"}]
}, async ({event, step})=>{
    const {orderId} = event.data;

    // waif 5 minutes before attempting assignment
    ...
    const result = await step.run("assign-rider", async ()=>{
        ...
        const busyRoderIds = busyOrders.map((o)=>o.deliveryPartnerId)

        const availableRider = await prisma.deliveryPartner.findFirst({
            where: {
                isActive: true,
                id: {notIn: busyRoderIds as string[]}
            }
        })
